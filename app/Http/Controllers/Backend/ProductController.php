...
class ProductController extends Controller
{
    ...
    public function barcode_print(Request $request,$id)
    {
        ...
        if($variants){
            ...
            return view('backend.invoices.barcode_print', compact('productstock'));
        }else{
            $product = Product::findOrFail($id);
            return view('backend.invoices.barcode_print', compact('product'));
        }
        // $product = PDF::loadView('backend.product.barcode_print',compact('product'))->setPaper('a4');
